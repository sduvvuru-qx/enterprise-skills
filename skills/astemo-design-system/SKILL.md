...
3. Tables: **brandRed `#B6001A` fill with white text** for header rows. Not gray. Not blue. Not black.
4. Alternating row shading: `#F2F2F2` on even rows.
5. Status fills: complete=`#C6EFCE`, in_progress=`#FFEB9C`, open=`#FFC7CE`, not_started=`#D9D9D9`.
6. Priority colors: P0=`#B6001A`, P1=`#2A579A`, P2=`#737373`.
7. Margins: 0.75" sides, 1" top/bottom (Letter: 12240×15840 DXA).
8. Shell: header = classification (brandRed) + "Astemo, Ltd." (muted) + red rule. Footer = copyright + page N of M.
9. Title: brandRed, 28pt bold, with 3pt red rule below.
10. CJK font fallback: Yu Gothic UI.

**Deterministic renderer:** `~/.agents/skills/copilot-canvas-renderer/`
- DOCX: `scripts/render-astemo-docx.cjs <payload.json> [output.docx]`
- PDF: `scripts/render-astemo-pdf.py <payload.json> [output.pdf]`
- Pipeline: `scripts/pipeline.sh payload.json --pdf --open`
- Visual regression: `scripts/visual-regression-test.py`

The renderer uses these exact tokens. Pixel-verified 9/9 design system compliance on all pages.
